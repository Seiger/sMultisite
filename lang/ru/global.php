<?php return [
    'action_cancelled' => 'Действие отменено.',
    'add_help' => 'Добавить новый домен',
    'site_color' => 'Цвет сайта',
    'check_fields' => 'Внимание! Проверьте поля.',
    'configure' => 'Конфигурация',
    'configure_help' => 'Настройка доменов сайта.',
    'configure_icon' => 'fa fa-sliders',
    'default_domain' => 'Домен по умолчанию:',
    'default_domain_help' => 'Название домена настраивается в Системной конфигурации сайта, таб Сайт. Этот домен будет задействован, если входной запрос не отвечает ни одному из тех, что приведены ниже.',
    'description' => 'Настройка дополнительных доменов сайта.',
    'domain' => 'Доменное имя:',
    'domain_help' => 'Укажите доменное имя без протокола.',
    'domain_on' => 'Включить домен:',
    'domain_on_help' => 'Если домен выключен, то доступ к страницам этого домена будет запрещен.',
    'domain_key' => 'Ключ домена',
    'domain_key_help' => 'При выборе ключа будьте внимательны, ключ вводится на английском в нижнем регистре и только буквы. <b>Внимание, ключ изменить нельзя</b>.',
    'empty_field' => 'поле не должно быть пустым.',
    'error_empty_fields' => 'Ошибка: Все поля должны быть заполнены!',
    'hex_color_may_be_used_some_modules' => 'Этот <b>HEX-цвет</b> может использоваться некоторыми модулями для обозначения ресурсов, принадлежащих выбранному домену.',
    'hide_from_tree' => 'Спрятать в дереве ресурсов:',
    'hide_from_tree_help' => 'Если это поле будет отмечено, то ресурсы домена не будут отражены в дереве ресурсов. Возможно, только если <b>домен выключен</b>.',
    'icon' => 'fa fa-dice-d20',
    'must_be_dis-active' => 'чтобы спрятать, домен домен должен быть выключен.',
    'no_valid_domain' => 'Не валидный домен или такой домен уже зарегистрирован.',
    'no_valid_domain_key' => 'Не валидный ключ домена или такой ключ уже зарегистрирован.',
    'not_saved' => 'Не сохранено.',
    'not_writable' => 'Файл <b>:file</b> не доступний для запису.',
    'success_updated' => 'Данные успешно обновлены!',
    'title' => 'Мультисайт',
    'warning' => 'Увага',
];
