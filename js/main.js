...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (selector) {
        const cookieName = 'sMultisitePerPage';
 ...
 */
window.sMultisite.queueLucide = function queueLucide() {
    if (window.lucide?.createIcons) {
        lucide.createIcons();
    } else {
 ...
 */
window.sMultisite.callApi = async function callApi(url, form = null, method = 'POST', type = 'json', headers = {}) {
    try {
        const finalHeaders = {
...
window.sMultisite.submitForm = function submitForm(selector) {
    ...
    if (form) {
        ...
        validates.forEach(function (element) {
            ...
            switch (rule[0]) {
                case "hideFromTreeCheck":
                    ...
                    if (element.value == 1 && document.getElementById('active_' + idx).value == 1) {
                        messages.push(element.getAttribute('data-text'));
                        element.parentNode.querySelector('.s-toggle-slider')?.classList.add('is-invalid');
                        element.parentNode.parentNode.querySelector('p')?.classList.add('text-invalid');
                    } else {
                        element.parentNode.querySelector('.s-toggle-slider')?.classList.remove('is-invalid');
                        element.parentNode.parentNode.querySelector('p')?.classList.remove('text-invalid');
                    }
                    break;
                case "textNoEmpty":
                    ...
                    if (element.value.length < 1) {
                        ...
                        element.parentNode.querySelector('p')?.classList.add('text-invalid');
                    } else {
                        element.classList.remove('is-invalid');
                        element.parentNode.querySelector('p')?.classList.remove('text-invalid');
        ...
        if (messages.length < 1) {
            ...
            alertify.alert(window.sMultisite.lang.check_fields, messages.join("<br/>"), function(){
                alertify.error(window.sMultisite.lang.not_saved)
            }).set('modal', true);
        }
    }
}
